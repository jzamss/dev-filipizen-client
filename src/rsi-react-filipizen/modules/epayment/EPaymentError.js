...
const EPaymentError = ({ partner, history, location }) => {
  ...
  if (partner.email) {
    contacts.push(
      contacts.length > 0
        ? `or email at ${partner.email}`
        : `Email us at ${partner.email}`
    );
  }
        ...
        <Card>
          <Content center>
            ...
            <Label labelStyle={styles.text}>
              Our partner was not able to process your payment.
            </Label>
            <Label labelStyle={styles.text}>
              Make sure to verify that your credentials are correct upon
