...
import {
  ...
  Subtitle,
  Spacer,
} from "../verification/node_modules/rsi-react-web-components";
...
const EPaymentSuccess = (props) => {
  ...
  if (partner.email) {
    contacts.push(
      contacts.length > 1
        ? `or email to ${partner.email}`
        : `For inquiries, email us on ${partner.email}`
    );
  }
        ...
        >
          ...
          <Panel style={styles.amountContainer}>
            ...
            <label style={styles.amount}>
              PHP {currencyFormat(payment.amount)}
            </label>
          </Panel>
          <Panel center>
            ...
            <Image src={`/assets/${payment.paypartnerid}.png`} width={150} />
            <Image />
          </Panel>
        </FormPanel>
        <Label style={{ ...styles.text, ...{ maxWidth: 300 } }}>
          Your e-receipt and tickets will be sent to your email at{" "}
          {payment.email}
        </Label>
        <Label labelStyle={styles.text}>Thank you for using this service</Label>
        {contacts.length > 0 && (
          <Label labelStyle={styles.text}>{contacts.join(" ")}</Label>
        )}
        <CardActions>
